...
class Approve extends Presenter
{
    ...
    public function get($id)
    {
        ...
        if ($addon->isApproved()) {
            ...
            $duplicate['namespace'] = ($this->persistence->build('Addon')->byShort($addon->getProposedShort())
                !== null);
            $duplicate['title_en'] = count($this->persistence->build('Addon')->all()->byTitle($addon->getTitle('en'),
                        0, 0, 'en')) > 1;
            $duplicate['title_de'] = count($this->persistence->build('Addon')->all()->byTitle($addon->getTitle('de'),
                        0, 0, 'de')) > 1;
        }
        ...
        $this->context['timestamp'] = date($this->localisation->getFormat(LocalisationService::FORMAT_DATETIME),
            $addon->getApprovalSubmit());

        if (!isset($this->context['comment'])) {
            $this->context['comment'] = $addon->getApprovalComment();
        }
        $this->context['comment_help'] = sprintf(gettext('Please comment in %s, the users language.'),
            $this->localisation->namedLanguage($owner->getLanguage()));
    ...
    public function post($id)
    {
        ...
        if ((isset($_POST['approve']) && $this->persistence->build('Addon')->byShort($addon->getProposedShort())
            !== null)) {
