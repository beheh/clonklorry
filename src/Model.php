...
abstract class Model
{
    ...
    final protected function byValues($pairs = array())
    {
        ...
        if ($this->multiple) {
            $rows = $this->persistence->loadAll($this, $pairs, $order,
                $this->limit_from, $this->limit);
            ...
            $row = $this->persistence->load($this, $pairs, $order,
                $this->limit_from, $this->limit);
    ...
    final public function unserialize($row)
    {
        ...
        foreach ($row as $key => $value) {
            $this->values[$key] = $this->ensureType($key,
                $this->decodeType($key, $value));
        }
